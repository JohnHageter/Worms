...
# Per-well trackers
trackers = {
    i: WellTracker(i, max_dist=500, max_missed=20000) for i in range(len(wells))
}
...
for video_path in video_paths:
    ...
    while True:
        ...
        for t in all_tracks:
            ...
            # draw centroid velocity
            # if t.prev_centroid is not None:
            #     cv2.arrowedLine(
            #         visual,
            #         (cx, cy),
            #         (int(cx + t.velocity[0]), int(cy + t.velocity[1])),
            #         (0, 255, 255),
            #         2,
            #         tipLength=0.3,
            #     )

            # draw head and its velocity
            if (
                t.head is not None
                and hasattr(t, "prev_head")
                and t.prev_head is not None
            ):
                hx, hy = int(t.head[0]), int(t.head[1])
                cv2.rectangle(
                    visual, (hx - 4, hy - 4), (hx + 4, hy + 4), (255, 0, 0), -1
                )
                head_vel = t.head - t.prev_head
                # cv2.arrowedLine(
                #     visual,
                #     (hx, hy),
                #     (int(hx + head_vel[0]), int(hy + head_vel[1])),
                #     (255, 255, 0),
                #     2,
                #     tipLength=0.3,
                # )

            # draw tail and its velocity
            if (
                t.tail is not None
                and hasattr(t, "prev_tail")
                and t.prev_tail is not None
            ):
                tx, ty = int(t.tail[0]), int(t.tail[1])
                cv2.circle(visual, (tx, ty), 3, (0, 0, 255), -1)
                tail_vel = t.tail - t.prev_tail
                # cv2.arrowedLine(
                #     visual,
                #     (tx, ty),
                #     (int(tx + tail_vel[0]), int(ty + tail_vel[1])),
                #     (0, 255, 255),
                #     2,
                #     tipLength=0.3,
                # )
